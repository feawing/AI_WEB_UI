...
class StatusAgentExecutor(AgentExecutor):
    """An AgentExecutor that runs Relocation status ADK-based Agent."""

    def __init__(self, runner: Runner):
        self.runner = runner
        self._running_sessions: dict[str, types.Session] = {}

    def _run_agent(
        self, session_id:str, new_message: types.Content
    ) -> AsyncGenerator[Event, None]:
        return self.runner.run_async(
            session_id=session_id, user_id="status_agent", new_message=new_message
        )
    ...
    async def _process_request(
        ...
        self._running_sessions[session_id] = session_obj
    ...
    async def _upsert_session(self, session_id: str):
        session = await self.runner.session_service.get_session(
            app_name=self.runner.app_name, user_id="status_agent", session_id=session_id
        )
        if session is None:
            session = await self.runner.session_service.create_session(
                app_name=self.runner.app_name,
                user_id="status_agent",
                session_id=session_id,
            )
        if session is None:
